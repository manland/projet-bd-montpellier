...
public class SQL {
	...
	public static void creerTable() throws SQLException
	{
		...
		try
		{
			...
			sql ="create or replace trigger tg_reservation_terrain_squash\n";
			...
			sql+= "hd := :new.heure_debut;\n";
			sql+= "hf := :new.heure_fin;\n";
			sql+= "md := :new.minute_debut;\n";
			sql+= "mf := :new.minute_fin;\n";
			sql+= "if(hf-hd=1) then\n";
			sql+= "if(mf=0) then\n";
			sql+= "res:= 60-md;\n";
			sql+= "if(res!=40) then\n";
			sql+= "raise_application_error(-20501,'erreur la reservation d'un terrain doit durer 40min');\n";
			sql+= "end if;\n";
			sql+= "elsif(md>mf) then\n";
			sql+= "res:=(60+mf)-md;\n";
			sql+= "if(res!=40) then\n";
			sql+= "raise_application_error(-20501,'erreur la reservation d'un terrain doit durer 40min');\n";
			sql+= "end if;\n";
			...
			sql+= "end;\n";
			//statement.executeQuery(sql);
			System.out.println("Trigger reservationTerrainSquash cr�e");
			c.commit();	
			...
			sql+= "select count(num_terrain_squash) into nb_terr_res_par_ad from personne p, reservationTerrainSquash r, adherent a where r.id_personne=a.id_personne and r.id_personne=p.id_personne and p.id_personne=a.id_personne and date_location=date_loc and heure_debut=hd and heure_fin=hf;";
			...
			sql+= "if(n!=0) then";
			sql+= "select count(*) into nb_id_pers from reservationTerrainSquash where id_personne=num_person and date_location=date_loc and ((heure_debut=hd  and (minute_debut=min_deb or (heure_fin=hf and minute_debut<min_deb) or (heure_debut=hf and minute_debut>min_deb and minute_fin<min_fin))) or (heure_debut<hd and hf=heure_fin and min_deb<minute_debut) or (heure_debut=hf and min_deb>minute_debut and (hf!=heure_debut and min_fin!=min_deb)) or (heure_debut>hd and heure_fin=hf and minute_debut<min_deb and minute_fin>min_fin and min_fin!=minute_debut) or (heure_fin<hf and heure_debut=hd and ((hd!=heure_fin and min_deb!=minute_fin) or (min_deb>minute_debut and min_deb<minute_fin))));";
			...
			sql+= "if(cpt!=0) then";
			sql+= "raise_application_error(-20501,'erreur ce terrain est deja reserver pour cet horraire (ou chevauchement des horraires)');";
			sql+= "end if;";
			sql+= "end if;";
			sql+= "exception";
			sql+= "when no_data_found then";
			...
			sql+= "min_deb := :new.minute_debut;";
			sql+= "min_fin := :new.minute_fin;";
			sql+= "jour_res := to_char(:new.date_location,'D')-2;";
			...
			sql+= "end;";
			//statement.executeQuery(sql);
			System.out.println("Trigger reservationTerrainSquash_res_cren cr�e");
			c.commit();	
			...
			sql +="if(jour_ind='mon') then";
			sql +="jour_inter:=0;";
			sql +="elsif (jour_ind='tue') then";
			sql +="jour_inter:=1;";
			sql +="elsif (jour_ind='wed') then";
			sql +="jour_inter:=2;";
			sql +="elsif (jour_ind='thu') then";
			sql +="jour_inter:=3;";
			sql +="elsif (jour_ind='fri') then";
			sql +="jour_inter:=4;";
			sql +="elsif (jour_ind='sat') then";
			sql +="jour_inter:=5;";
			sql +="elsif (jour_ind='sun') then";
			sql +="jour_inter:=6;";
			sql +="end if;";
			...
			sql +="date_fin_trimestre date;";
			sql +="jour_trim varchar2(2);";
			sql +="idd inscriptionCoursSqCollectif.id_cours%TYPE;";
			sql +="i int;";
			sql +="nb_terr_res int;";
			sql +="CURSOR curseur is select jour,heure_debut,heure_fin,debut_trimestre,to_number(to_char(debut_trimestre,'YYYY')) from creneauCoursSqColl where  id_cours=id_c;";
			sql +="begin";
			sql +="i:=0;";
			sql +="id_c:=:new.id_cours;";
			sql +="num_ad := :new.num_adherent;";
			sql +="open curseur;";
			sql +="loop";
			sql +="FETCH curseur into jour_c,hd,hf,trimestre,annee;";
			...
			sql +="select count(*) into nb_terr_res from reservationTerrainSquash where num_terrain_squash=num_terr and to_char(date_location,'D')=jour_c+2 and ((heure_debut=hd or heure_debut=hd+1) or ((hf=heure_debut and minute_fin!=0) or hf=heure_debut+1));";
...
//			statement.executeQuery(sql);
			...
			sql +="(";
			sql +="id_salle int,";
			sql +="check(id_salle between 0 and 9),";
			sql +="capacite int,";
...
//			statement.executeQuery(sql);
			...
			/*sql = "INSERT INTO forfait VALUES('Salle de Musculation',0)";
			statement.executeQuery(sql);
			c.commit();sql = "INSERT INTO forfait VALUES('Cours de gymnastique',0)";
			statement.executeQuery(sql);
			c.commit();sql = "INSERT INTO forfait VALUES('Terrain de squash',0)";
			statement.executeQuery(sql);
			c.commit();sql = "INSERT INTO forfait VALUES('Cours particulier de squash',0)";
			statement.executeQuery(sql);
			c.commit();sql = "INSERT INTO forfait VALUES('Cours groupe de squash',0)";
			statement.executeQuery(sql);
			c.commit();
*/
...
//			
			sql = "insert into forfait(type_forfait,prix_base) values ('salle muscu',40)";
			statement.executeQuery(sql);
			c.commit();
			sql = "insert into forfait(type_forfait,prix_base) values ('cours gymnastique',40)";
			statement.executeQuery(sql);
			c.commit();
			sql = "insert into forfait(type_forfait,prix_base) values ('terrain squash',40)";
			statement.executeQuery(sql);
			c.commit();
			sql = "insert into forfait(type_forfait,prix_base) values ('cours particulier squash',40)";
			statement.executeQuery(sql);
			c.commit();
			sql = "insert into forfait(type_forfait,prix_base) values ('cours groupe squash',40)";
			statement.executeQuery(sql);
			c.commit();
...
//			statement.executeQuery(sql);
//			c.commit();
//			sql = "insert into souscription(type_forfait,num_adherent,date_debut,date_fin) values ('cours groupe squash',1,'5-01-2010','04-02-2010')";
//			statement.executeQuery(sql);
//			c.commit();
...
//			statement.executeQuery(sql);
//			c.commit();
//			sql = "insert into creneauCoursSqColl(id_cours,num_terrain_squash,jour,heure_debut,heure_fin,debut_trimestre) values (9,3,1,10,12,'01-01-2010')";
//			statement.executeQuery(sql);
//			c.commit();
...
//
//			sql = "insert into creneauCoursGym(id_salle,type_cours,date_cours,heure_debut,heure_fin,debut_trimestre) values(1,'step','4-07-2009',14,15,'01-01-2010')";
//			statement.executeQuery(sql);
//			c.commit();
//			sql = "insert into creneauCoursGym(id_salle,type_cours,date_cours,heure_debut,heure_fin,debut_trimestre) values(2,'cardio','4-07-2009',14,15,'01-01-2010')";
//			statement.executeQuery(sql);
//			c.commit();
//			sql = "insert into creneauCoursGym(id_salle,type_cours,date_cours,heure_debut,heure_fin,debut_trimestre) values(3,'cardio','4-07-2009',14,15,'01-01-2010')";
//			statement.executeQuery(sql);
//			c.commit();
